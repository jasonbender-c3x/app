  ...
  app.post("/api/chats/:id/messages", async (req, res) => {
    ...
    try {
      ...
      for await (const chunk of result) {
        ...
        if (text) {
          ...
          if (parseResult.proseToStream) {
            res.write(`data: ${JSON.stringify({ text: parseResult.proseToStream })}\n\n`);
          }
      ...
      if (flushResult.proseToStream) {
        res.write(`data: ${JSON.stringify({ text: flushResult.proseToStream })}\n\n`);
      }
      ...
      let finalContent = cleanContentForStorage
        ...
        .replace(/\[\s*\{[\s\S]*?"type"\s*:\s*"(?:github_|gmail_|calendar_|drive_|docs_|sheets_|tasks_|terminal_|tavily_|perplexity_|browserbase_|api_call|search|web_search|google_search|duckduckgo_search|browser_scrape|file_ingest|file_upload)[\w_]*"[\s\S]*?\}\s*\]/gi, '')
        ...
        .replace(/\{\s*"type"\s*:\s*"(?:github_|gmail_|calendar_|drive_|docs_|sheets_|tasks_|terminal_|tavily_|perplexity_|browserbase_|api_call|search|web_search|google_search|duckduckgo_search|browser_scrape|file_ingest|file_upload)[\w_]*"\s*,[\s\S]*?\}/gi, '')
        .replace(/"(?:operation|parameters|id|type)"\s*:\s*(?:"[^"]*"|\{[^}]*\})/gi, '')
...
${summary}`
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // WEBSOCKET: GEMINI LIVE API (REAL-TIME AUDIO STREAMING)
  // Provides low-latency voice responses via WebSocket connection
  // ═════════════════════════════════════════════════════════════════════════
  ...
  wss.on("connection", async (ws, req) => {
    ...
    ws.on("message", async (data) => {
      ...
      try {
        ...
        switch (message.type) {
          case "connect": {
            ...
            try {
              ...
              liveSession.emitter.on("turnComplete", () => {
                if (ws.readyState === WebSocket.OPEN) {
                  ws.send(JSON.stringify({ type: "end" }));
                }
              });
              
              liveSession.emitter.on("error", (error: any) => {
                if (ws.readyState === WebSocket.OPEN) {
          ...
          case "message": {
            if (!liveSession) {
              ws.send(JSON.stringify({ type: "error", message: "Session not connected. Send 'connect' first." }));
              return;
            }
            
            try {
              await sendTextMessage(liveSession, message.text);
            } catch (error: any) {
              console.error("[Live API] Stream error:", error);
              ws.send(JSON.stringify({ type: "error", message: error.message }));
            }
            break;
          }
        ...
        console.error("[Live API] Message parse error:", error);
  ...
  console.log("[Live API] WebSocket server ready at /api/live");
